...
class RuiyanHandStatusMessage:
    ...
    def print(self):
        print(f"Motor ID: {self.motor_id}, Instruction: {self.instruction}, Status: {self.status}, Position: {self.position}, Velocity: {self.velocity}, Current: {self.current}")
...
class SerialInterface(CommunicationInterface):
    ...
    def __init__(self, port: str , baudrate: int ,timeout: float = 0.01, auto_connect: bool = False, mock:bool=False):
        self.port = port
    ...
    def connect(self) -> bool:
        ...
        try:
            ...
            self.connected = False
            logger.error("Serial library not installed, please install pyserial: pip install pyserial")
            return False
    ...
    def _send_message(self, message: RuiyanHandControlMessage) -> bool:
        ...
        if self.mock == True:
            ...
            logger.debug(f"Send - Motor ID: {message.motor_id}, Instruction: {hex(message.instruction)}, Frame data: {' '.join([f'{byte:02X}' for byte in serial_frame])}")
            return True
