...
class SineWavePublisher(Node):
    def __init__(self):
        ...
        self.left_hand_publisher = self.create_publisher(
            JointState, "/ruiyan_hand/left/set_angles", 10
        )

        self.right_hand_publisher = self.create_publisher(
            JointState, "/ruiyan_hand/right/set_angles", 10
        )
    ...
    def publish_sine_wave(self):
        ...
        for i, joint_idx in enumerate(self.controlled_joints):
            ...
            sine_value = math.sin(
                2 * math.pi * self.frequency * current_time + phase_offset
            )
            position = float(
                int(self.amplitude * sine_value + self.amplitude + 0.5)
            )
        ...
        if int(current_time * 10) % 1 == 0:
            controlled_positions = [
                positions[i] for i in self.controlled_joints
            ]
            self.get_logger().info(
                f"Time: {current_time:.1f}s, "
                f"Controlled joint positions: {controlled_positions}"
            )
